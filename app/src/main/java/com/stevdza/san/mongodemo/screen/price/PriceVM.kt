...
class PriceVM: ViewModel(){
    ...
    fun getPriceList() {
        viewModelScope.launch {
            AlMurabbiDB.getPriceList().collect {
                priceList.value = it
            }
        }
    }
    ...
    fun insertPrice() {
        viewModelScope.launch(Dispatchers.IO) {
            if (
                ...
                AlMurabbiDB.insertPrice(price = Price().apply {
    ...
    fun updatePriceList() {
        viewModelScope.launch(Dispatchers.IO) {
            if (objectId.value.isNotEmpty()){
                AlMurabbiDB.updatePrice(price = Price().apply {
    ...
    fun deleteBook() {
        viewModelScope.launch {
            if (objectId.value.isNotEmpty()) {
                AlMurabbiDB.deletePrice(id = BsonObjectId(hexString = objectId.value))
            }
        }
    }
    ...
    fun filterWithBookName() {
        viewModelScope.launch(Dispatchers.IO) {
            AlMurabbiDB.filterWithBookName(bookName = bookName.value).collect{
                priceList.value = it
            }
        }
    }
