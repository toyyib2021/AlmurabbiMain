...
class BookRecordVM : ViewModel(){
    ...
    fun fechtAllBookRecord() {
        viewModelScope.launch {
            AlMurabbiDB.getBookProduced().collect {
                books.value = it
            }
        }
    }
    ...
    fun insertBookRecord() {
        viewModelScope.launch(Dispatchers.IO) {
            if (bookName.value.isNotEmpty()) {
                AlMurabbiDB.insertBookRecord(bookProduced = BookProduced().apply {
    ...
    fun updateBookRecord() {
        viewModelScope.launch(Dispatchers.IO) {
            if (objectId.value.isNotEmpty()){
                AlMurabbiDB.updateBookRecord(bookProduced = BookProduced().apply {
    ...
    fun deletePerson() {
        viewModelScope.launch {
            if (objectId.value.isNotEmpty()) {
                AlMurabbiDB.deleteBookRecord(id = BsonObjectId(hexString = objectId.value))
            }
        }
    }
    ...
    fun filterData(): String {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            AlMurabbiDB.filterData(bookName = bookName.value).collect{
                books = it
            }
        }
