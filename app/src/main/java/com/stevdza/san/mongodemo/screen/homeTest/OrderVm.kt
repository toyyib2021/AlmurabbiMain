...
class OrderVm : ViewModel() {
    ...
    fun insertOrder() {
        viewModelScope.launch(Dispatchers.IO) {
            if (
                ...
                AlMurabbiDB.insertOrder(order = Order().apply {
    ...
    fun updateOrder() {
        viewModelScope.launch(Dispatchers.IO) {
            if (objectId.value.isNotEmpty()) {
                AlMurabbiDB.updateOrder(order = Order().apply {
    ...
    fun getOrderListById() {
        viewModelScope.launch(Dispatchers.IO) {
            AlMurabbiDB.getOrderWithID(_id = ObjectId(hexString = this@OrderVm.objectId.value)).let {
                orderById.value = it
            }
        }
    ...
    fun filterOrderWithSchoolName(){
        viewModelScope.launch(Dispatchers.IO) {
            AlMurabbiDB.filterOrderWithSchoolName(schoolName = schoolName.value).collect {
                order.value = it
            }
        }
    }
