...
class CustomerVM : ViewModel(){
    ...
    fun getAllCustomer() {
        viewModelScope.launch {
            AlMurabbiDB.getCustomer().collect {
                customers.value = it
            }
        }
    }
    ...
    fun insertCustomer() {
        viewModelScope.launch(Dispatchers.IO) {
            if (
                ...
                AlMurabbiDB.insertCustomer(customer = Customer().apply {
    ...
    fun updateCustomer() {
        viewModelScope.launch(Dispatchers.IO) {
            if (objectId.value.isNotEmpty()){
                AlMurabbiDB.updateCustomer(customer = Customer().apply {
    ...
    fun deleteCustomer() {
        viewModelScope.launch {
            if (objectId.value.isNotEmpty()) {
                AlMurabbiDB.deleteCustomer(id = BsonObjectId(hexString = objectId.value))
            }
        }
    }
    ...
    fun filterWithSchoolName() {
        viewModelScope.launch(Dispatchers.IO) {
            AlMurabbiDB.filterWithSchoolName(schoolName = schoolNameSearch.value).collect{
                customers.value = it
            }
        }
    }
    ...
    fun getCustomer(schoolName: String) {
        viewModelScope.launch(Dispatchers.IO) {
            AlMurabbiDB.getCustomer(schoolName = schoolName).let { customer ->
